...
def bn_init_as_tf(m):
    if isinstance(m, nn.BatchNorm2d):
        ...
        m.eval()                      # or zero mean and unit variance if the batch norm layer has no pretrained stats
        m.affine = True
...
class silog_loss(nn.Module):
    ...
    def forward(self, depth_est, depth_gt, mask):
        ...
        silog = torch.sqrt((d ** 2).mean() - 0.85 * (d.mean() ** 2)) * 10.0
        return silog
...
class reduction_1x1(nn.Sequential):
    ...
    def forward(self, net):
        ...
        if not self.is_final:
            theta = self.sigmoid(net[:, 0, :, :]) * math.pi / 6
...
class bts(nn.Module):
    def __init__(self, params, feat_out_channels, num_features=512):
        ...
        self.daspp_conv = torch.nn.Sequential(nn.Conv2d(num_features + num_features // 2 + num_features // 4, num_features // 4, 3, 1, 1, bias=False),
                                              nn.ELU())
        self.reduc8x8   = reduction_1x1(num_features // 4, num_features // 8, self.params.max_depth)
    ...
    def forward(self, features, focal):
        ...
        upconv5 = self.upconv5(features[5]) # H/16
...
class encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for k, v in self.base_model._modules.items():
            if 'resnet' in self.params.encoder and 'fc' in k:
                continue
...
class BtsModel(nn.Module):
    ...
    def forward(self, x, focal):
        dense_features = self.encoder(x)
        return self.decoder(dense_features, focal)
